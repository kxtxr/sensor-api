...
import { faker } from '@faker-js/faker';  // Update this import
...
async function seed() {
  ...
  const sensorTypes = ['temperature', 'humidity', 'pressure', 'light', 'motion'];
  ...
  for (const type of sensorTypes) {
    ...
    const sensorData = Array(100).fill(null).map(() => {
      ...
      data.recorded_at = faker.date.between('2023-01-01T00:00:00.000Z', '2024-07-23T00:00:00.000Z');
...
function generateRealisticValue(sensorType: string): string {
  switch (sensorType) {
    case 'temperature':
      return faker.number.float({ min: -10, max: 40, precision: 0.1 }).toFixed(1);
    case 'humidity':
      return faker.number.float({ min: 0, max: 100, precision: 0.1 }).toFixed(1);
    case 'pressure':
      return faker.number.float({ min: 950, max: 1050, precision: 0.1 }).toFixed(1);
    case 'light':
